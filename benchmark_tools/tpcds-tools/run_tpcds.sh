...
run_dbgen() {
  ...
  #"${run_dbgen_path}/dsdgen" -scale $scale_value -dir $destination -suffix .csv -terminate n
...
run_benchmark() {
  ...
  case $database_type in
    "duckdb")
      sudo sh -c 'sync && echo 3 > /proc/sys/vm/drop_caches'
      python3 main.py --benchmark tpcds --database duckdb --data $data_dir --db_path $db_path
      if [ "$?" -ne 0 ]; then
        echo "DuckDB is OOM"
        memory_info=$(free -m)
	...
	python3 main.py --benchmark tpcds --database duckdb --data $data_dir --db_path $db_path --mem_limit $available_memory_gb
        for mem_limit in "${memory_limits[@]}"; do
          ...
          sudo sh -c 'sync && echo 3 > /proc/sys/vm/drop_caches'
          python3 main.py --benchmark tpcds --database duckdb --data $data_dir --db_path $db_path --mem_limit $mem_limit
        done
      fi
      ;;
    "hyper")
      sudo sh -c 'sync && echo 3 > /proc/sys/vm/drop_caches'
      python3 main.py --benchmark tpcds --database hyper --data $data_dir --db_path $db_path
      ;;
    "monetdb")
      sudo sh -c 'sync && echo 3 > /proc/sys/vm/drop_caches'
      check_and_create_monetdb_files_dir "$db_path"
      python3 main.py --benchmark tpcds --database monetdb --data $data_dir --dbname tpcds --dbfarm "${db_path}/monetdb_files"
      ;;
    "starrocks")
     #Define the path for Starrocks files !!!!!!!!!!!!!!
      ...
      cd $python_script_path
      sudo sh -c 'sync && echo 3 > /proc/sys/vm/drop_caches'
      python3 main.py --benchmark tpcds --database starrocks --ip 127.0.0.1 --data $data_dir --name tpcds
      cd $starrocks_path
...
cd $python_script_path
cd "scripts"
results_path="$python_script_path/core/benchmarks/tpcds/results/csv/"
python3 plotResults.py --csv_path $results_path --benchmark tpcds
